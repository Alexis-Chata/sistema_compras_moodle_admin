...
class GestionarUsuarios extends Component
{
    ...
    public function modal($user_id = null ){
        $this->reset('roles2','perfil_imagen');
        $this->iteration++;
        if($user_id == null)
        {
            $this->user = new User();
    ...
    public function save()
    {
        ...
        $this->user->save();
        #asignar roles
        $this->eliminar_roles($this->user);
        $this->agregar_roles($this->user);
    ...
    public function eliminar_roles(User $user){
        foreach($user->roles as $rola){
            $user->removeRole($rola->id);
            if($user->id == 1)
            {
    ...
    public function render()
    {
        ...
        return view('livewire.gestionar-usuarios',compact('usuarios','roles'));
    }
}
