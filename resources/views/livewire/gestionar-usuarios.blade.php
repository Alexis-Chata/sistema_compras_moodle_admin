                    ...
                    <div class="row m-1">
                        ...
                        <div class="col-3 pb-4">
                            ...
                            <select class="form-select" wire:model="n_pagina">
                                ...
                                <option value="{{$usuarios->total()}}">Todo</option>
                            </select>
                        </div>
                    </div>
        </div>
    ...
    <div class="card card-secondary">
        <div class="card-header" style="background: #0b4d87">
            <div class="row align-items-center p-1">
                <div class="col col-md-3">
                    <h5>Listado de Usuarios : {{$usuarios->total()}}</h5>
                </div>
            </div>
        </div>
        <div class="card-body p-inherit">
                    ...
                    <tbody class="table-secondary">
                        @foreach ($usuarios as $key => $usuario)
                            <tr>
                                ...
                                <td>{{$usuario->name." ".$usuario->ap_paterno." ".$usuario->ap_materno }}</td>
                                <td>{{$usuario->codigo }}</td>
                                <td>{{$usuario->email }}</td>
                                <td>{{$usuario->dni}}</td>
                                <td>{{$usuario->celular}}</td>
                                <td>@foreach ($usuario->roles as $role){{$role->name}} @endforeach</td>
                                <td>
                                    <button class="btn btn-success" id="editar-{{ $usuario->id }}" data-bs-toggle="modal"
                                        data-bs-target="#usuario_modal"
                                        wire:click="modal({{ $usuario->id }})"><i class="bi bi-pencil-square"></i>
                                    </button>
                                        <button class="btn btn-dark" id="reiniciar-{{ $usuario->id }}"
                                        wire:click="$emit('reiniciar_usuario',{{ $usuario->id }})"><i class="bi bi-arrow-clockwise"></i></button>
                                        <button class="btn btn-danger" id="eliminar-{{ $usuario->id }}"
                                        wire:click="$emit('eliminar_usuario',{{ $usuario->id }})"><i class="bi bi-trash-fill"></i></button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="usuarioModalLabel">{{ $modal_titulo }} - Usuario</h5>
                                            ...
                                            <div class="row m-4">
                                                ...
                                                <div class="col-12">
                                                    ...
                                                    <input type="file" id="perfil_imagen-{{$iteration}}" class="form-control" wire:model="perfil_imagen">
                                                        ...
                                                        @if ($u_imagen_perfil)
                                                        <div class="text-center"> Imagen del Usuario<br>
